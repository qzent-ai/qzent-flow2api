...
from ..core.log_operations import generation_operation
...
from ..services.flow_web import FlowWebError
...
class UpdateTokenRequest(BaseModel):
    st: str = ""  # 旧模式使用 ST，新版模式使用 google_cookies
    project_id: Optional[str] = None  # 用户可选输入project_id
    project_name: Optional[str] = None
    remark: Optional[str] = None
    captcha_proxy_url: Optional[str] = None
...
async def _save_flow_web_settings(token_id, request):
    # Only noncredential settings: cookies were filtered and validated by sync.
    fields = {key: getattr(request, key) for key in (
        "remark", "project_id", "project_name", "captcha_proxy_url", "extension_route_key",
        "image_enabled", "video_enabled", "image_concurrency", "video_concurrency",
        "auto_refresh_enabled", "refresh_interval_minutes",
    ) if getattr(request, key, None) is not None}
    await token_manager.update_token(token_id=token_id, **fields)
    if concurrency_manager:
        updated = await token_manager.get_token(token_id)
        if updated:
            await concurrency_manager.reset_token(token_id, image_concurrency=updated.image_concurrency,
                                                  video_concurrency=updated.video_concurrency)
...
async def add_token(
    ...
    """Add a new token"""
    if request.protocol_mode == "flow_web":
        try:
            new_token, action = await token_manager.sync_flow_web_token(
                request.google_cookies or "", proxy_url=request.proxy_url,
            )
            await _save_flow_web_settings(new_token.id, request)
            return {"success": True, "message": "新版 Flow 会话已验证并同步", "action": action,
                    "token": {"id": new_token.id, "email": new_token.email, "project_id": new_token.current_project_id}}
        except FlowWebError as e:
            raise HTTPException(status_code=e.status_code, detail=str(e)) from None
    if not request.st.strip():
        raise HTTPException(status_code=400, detail="请输入 Session Token，或选择新版 Flow Cookie 模式")
    try:
...
async def update_token(
    ...
    if request.protocol_mode == "flow_web":
        try:
            await token_manager.sync_flow_web_token(request.google_cookies or "", expected_id=token_id,
                                                    proxy_url=request.proxy_url, auto_enable=False)
            await _save_flow_web_settings(token_id, request)
            return {"success": True, "message": "新版 Flow 会话已验证并更新"}
        except FlowWebError as e:
            raise HTTPException(status_code=e.status_code, detail=str(e)) from None
    try:
        # 先ST转AT
...
async def refresh_at(
    ...
    try:
        ...
        if success:
            ...
            message = "新版 Flow 会话已验证" if updated_token.protocol_mode == "flow_web" else "AT刷新成功"
...
async def import_tokens(
    ...
    for idx, item in enumerate(request.tokens):
        try:
            if item.protocol_mode == "flow_web":
                synced, action = await token_manager.sync_flow_web_token(item.google_cookies or "", proxy_url=item.proxy_url, auto_enable=False)
                await _save_flow_web_settings(synced.id, item)
                if not item.is_active:
                    await token_manager.disable_token(synced.id)
                if action == "added":
                    added += 1
                else:
                    updated += 1
                existing_by_email[synced.email] = synced
                continue
...
def _get_log_operation(log):
    """Recognize old generic operations only when stored metadata is conclusive."""
    operation = log.get("operation")
    if operation not in {"generate_image", "generate_video"}:
        return operation
    from ..services.generation_handler import MODEL_CONFIG
    model = log.get("request_model")
    has_images = log.get("request_has_images")
    if "request_body" in log:
        try:
            payload = json.loads(log.get("request_body") or "{}")
    ...
    model_config = MODEL_CONFIG.get(model) if isinstance(model, str) else None
    if not model_config or operation != "generate_" + model_config.get("type", ""):
        return operation
    return generation_operation(model_config, has_images is True or type(has_images) is int and has_images == 1)
...
async def get_logs(
    ...
    for log in logs:
        ...
        result.append({
            ...
            "operation": _get_log_operation(log),
...
async def get_plugin_config(request: Request, token: str = Depends(verify_admin_token)):
    ...
    if host_header:
        ...
        connection_url = f"{request.url.scheme}://{host_header}/api/plugin/update-token"
    else:
...
COOKIE_SYNC_ERRORS = {
    "flow_session_expired": "Flow 会话已失效，请重新登录并同步。",
    "flow_cookies_refresh_required": "请同步当前浏览器的最新 Cookie。",
    "flow_session_unverified": "暂时无法验证 Flow 会话，请稍后重试。",
    "flow_account_mismatch": "当前登录账号与绑定账号不一致。",
    "invalid_token_id": "绑定的账号 ID 无效。",
    "flow_access_denied": "Google 拒绝访问，请检查账号和网络。",
    "flow_connection_failed": "连接 Google 失败，请检查网络后重试。",
    "invalid_google_cookies": "Cookie 格式无效，请重新同步。",
    "flow_http_error": "Google 请求失败，请稍后重试。",
    "flow_response_invalid": "无法解析 Google 的响应，请检查服务端适配。",
    "flow_rpc_failed": "Google 会话接口调用失败，请稍后重试。",
}


@router.get("/api/cookie-sync/logs")
async def get_cookie_sync_logs(limit: int = 100, token: str = Depends(verify_admin_token)):
    return await db.get_cookie_sync_logs(limit=max(1, min(limit, 1000)))


@router.post("/api/plugin/update-token")
async def plugin_update_token(request: dict, authorization: Optional[str] = Header(None)):
    started = time.monotonic()
    fields = dict(account=None, source="plugin_api", action="failed", success=False,
                  status_code=500, error_code="sync_failed", message="同步失败，请检查服务端运行状态。")
    result = None
    try:
        result = await _plugin_update_token(request, authorization)
        if result.get("success"):
            action = result.get("action")
            fields.update(success=True, status_code=200, error_code=None,
                          action=action if action in {"added", "updated", "unchanged"} else "updated",
                          message="会话同步成功。")
        return result
    except HTTPException as exc:
        code = exc.detail.get("code") if isinstance(exc.detail, dict) else None
        message = COOKIE_SYNC_ERRORS.get(code) if isinstance(code, str) else None
        fields.update(status_code=exc.status_code, error_code=code if message else "sync_failed",
                      message=message or ("连接认证失败，请检查插件连接配置。" if exc.status_code in (401,403) else "同步失败，请检查登录状态和服务端配置。"))
        raise
    finally:
        # Never persist request bodies, credentials, or arbitrary upstream messages.
        try:
            if result and result.get("token_id"):
                account = await db.get_token(result["token_id"])
                if account:
                    fields["account"] = account.email
            fields["duration"] = round(time.monotonic() - started, 3)
            await db.add_cookie_sync_log(**fields)
        except Exception:
            pass  # Auditing must not turn a successful cookie update into a failure.


async def _plugin_update_token(request: dict, authorization: Optional[str] = Header(None)):
    ...
    if request.get("protocol_mode") == "flow_web":
        try:
            binding = {}
            if request.get("expected_token_id") is not None:
                expected = request["expected_token_id"]
                if not isinstance(expected, int) or isinstance(expected, bool) or expected < 1:
                    raise FlowWebError("invalid_token_id", "绑定的账号 ID 无效")
                binding["expected_id"] = expected
            updated, action = await token_manager.sync_flow_web_token(
                request.get("google_cookies", ""), auto_enable=plugin_config.auto_enable_on_update, **binding,
            )
            return {"success": True, "action": action, "token_id": updated.id,
                    "protocol_mode": "flow_web", "message": f"新版 Flow 会话已验证并同步：{updated.email}"}
        except FlowWebError as e:
            raise HTTPException(status_code=e.status_code, detail={"code": e.code, "message": str(e)}) from None
    ...
    if at_expires and (at_expires.replace(tzinfo=timezone.utc) if at_expires.tzinfo is None else at_expires) <= datetime.now(timezone.utc):
        raise HTTPException(status_code=401, detail={
            "code": "legacy_session_expired",
            "message": "旧版 Labs 会话返回了已过期的访问令牌，请在新版 Flow 登录后使用新版 Cookie 模式同步。错误码：legacy_session_expired",
        })
