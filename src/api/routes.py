...
def _parse_edit_directive(uri: str) -> Dict[str, Any]:
    """Parse edit://<mediaId>?start_frame=0&end_frame=240 into a directive dict."""
    rest = uri[len("edit://"):]
    media_id, _, query = rest.partition("?")
    if not media_id:
        raise HTTPException(status_code=400, detail="edit:// requires a source media id")
    params = parse_qs(query)
    try:
        start_frame = int(params.get("start_frame", ["0"])[0])
        end_frame = int(params.get("end_frame", ["240"])[0])
    except ValueError:
        raise HTTPException(status_code=400, detail="edit:// frame params must be integers")
    if not (0 <= start_frame < end_frame <= 300):
        raise HTTPException(status_code=400, detail="edit:// frame range must satisfy 0 <= start < end <= 300")
    return {"media_id": media_id, "start_frame": start_frame, "end_frame": end_frame}


# 视频上传:与 Flow 网页端一致的封装白名单;上限与 qzent 资产上限对齐(50MB)
...
def _parse_video_upload_request(
    video: Optional[str], video_url: Optional[str]
) -> tuple[Optional[bytes], Optional[str]]:
    """Validate the /v1/videos/uploads payload.

    Returns (data, content_type) for inline data URLs, or (None, None) when a
    remote video_url should be fetched by the caller. Raises ValueError on
    any validation failure.
    """
    if bool(video) == bool(video_url):
        raise ValueError("exactly one of video (data URL) or video_url is required")
    if video_url:
        return None, None
    header, separator, encoded = video.partition(",")
    if not separator or not header.startswith("data:") or not header.endswith(";base64"):
        raise ValueError("video must be a base64 data URL")
    mime = header[len("data:"):-len(";base64")]
    if mime not in VIDEO_UPLOAD_ALLOWED_MIMES:
        raise ValueError(f"unsupported video content type: {mime}")
    try:
        data = base64.b64decode(encoded, validate=True)
    except (ValueError, TypeError) as exc:
        raise ValueError("video data URL contains invalid Base64") from exc
    if len(data) > VIDEO_UPLOAD_MAX_BYTES:
        raise ValueError("video exceeds the 50MB upload limit")
    return data, mime


async def _fetch_video_url(video_url: str) -> tuple[bytes, str]:
    """Fetch a remote video with the upload size cap enforced."""
    async with httpx.AsyncClient(
        follow_redirects=True, trust_env=False, timeout=120
    ) as client:
        async with client.stream("GET", video_url) as resp:
            if resp.status_code != 200:
                raise ValueError(f"failed to fetch video_url: HTTP {resp.status_code}")
            chunks = []
            total = 0
    ...
    if content_type not in VIDEO_UPLOAD_ALLOWED_MIMES:
...
async def _extract_prompt_and_images_from_openai_messages(
    ...
    """Extract prompt, images, and optional video_media_id/edit_directive from messages.

    Returns:
        (prompt, images, video_media_id, edit_directive)
        video_media_id is set when an image_url starts with "extend://"
        edit_directive is set when an image_url starts with "edit://"
    """
    ...
    if isinstance(content, str):
        ...
        for item in content:
            ...
            if item_type == "text":
                ...
                # edit://MEDIA_ID?start_frame=&end_frame= 用于视频物体/场景编辑
                elif image_url.startswith("edit://"):
                    edit_directive = _parse_edit_directive(image_url)
                else:
                    images.append(await _load_image_bytes_from_uri(image_url))

    prompt = "\n".join(part for part in prompt_parts if part).strip()
    return prompt, images, video_media_id, edit_directive
...
async def _normalize_openai_request(
    ...
    if request.messages:
        prompt, images, video_media_id, edit_directive = await _extract_prompt_and_images_from_openai_messages(
            request.messages
        )
...
async def _iterate_openai_stream(
    ...
    async for chunk in handler.handle_generation(
        ...
        edit_directive=normalized.edit_directive,
    ):
...
class VideoUploadRequest(BaseModel):
    """Payload for POST /v1/videos/uploads."""

    video: Optional[str] = None  # base64 data URL (data:video/mp4;base64,...)
    video_url: Optional[str] = None  # 公网可访问的视频 URL
...
async def upload_video(
    ...
    """Upload an external video to Flow and return its mediaId (for edit://)."""
    handler = _ensure_generation_handler()
    try:
        data, content_type = _parse_video_upload_request(request.video, request.video_url)
        if data is None:
            data, content_type = await _fetch_video_url(request.video_url)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc))
    try:
        media_id = await handler.upload_video_for_edit(data, content_type)
    except Exception as exc:
