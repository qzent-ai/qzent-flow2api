...
class Database:
    ...
    async def _ensure_config_rows(self, db, config_dict: dict = None):
        ...
        if count[0] == 0:
            ...
            if config_dict:
                ...
                api_key = global_config.get("api_key") or api_key
        ...
        async with self._connect(write=True) as db:
            ...
            if await self._table_exists(db, "tasks"):
                tasks_columns_to_add = [
                    ("media_generation_id", "TEXT"),  # 生成结果的 mediaGenerationId (编辑/续写亲和用)
                ]
                ...
                # mediaGenerationId 反查属主 token (编辑/续写亲和) 走该索引
                await db.execute("CREATE INDEX IF NOT EXISTS ix_tasks_media_generation_id ON tasks(media_generation_id)")
    ...
    async def init_db(self):
        ...
        async with self._connect(write=True) as db:
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS cookie_sync_logs (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    account TEXT, source TEXT NOT NULL, action TEXT NOT NULL,
                    success INTEGER NOT NULL, status_code INTEGER NOT NULL,
                    duration REAL NOT NULL, error_code TEXT, message TEXT NOT NULL
                )
            """)
    ...
    async def get_media_result_urls(self, media_id: str) -> list:
        """Find a cached source without fetching any upstream credentials or URL."""
        async with self._connect() as db:
            cursor = await db.execute(
                "SELECT result_urls FROM tasks WHERE media_generation_id = ? OR task_id = ? ORDER BY created_at DESC LIMIT 1",
                (media_id, media_id),
            )
            row = await cursor.fetchone()
            try:
    ...
    async def get_token_id_by_media_generation_id(self, media_id: str) -> Optional[int]:
        """按 Flow mediaGenerationId 反查生成它的 token id(用于编辑/续写亲和)"""
        async with self._connect() as db:
            cursor = await db.execute(
                "SELECT token_id FROM tasks WHERE media_generation_id = ? ORDER BY id DESC LIMIT 1",
                (media_id,),
            )
            row = await cursor.fetchone()
    ...
    async def add_cookie_sync_log(self, **fields):
        async with self._connect(write=True) as db:
            await db.execute(
                "INSERT INTO cookie_sync_logs (account,source,action,success,status_code,duration,error_code,message) VALUES (?,?,?,?,?,?,?,?)",
                tuple(fields[key] for key in ('account','source','action','success','status_code','duration','error_code','message')),
            )
            await db.execute("DELETE FROM cookie_sync_logs WHERE id NOT IN (SELECT id FROM cookie_sync_logs ORDER BY id DESC LIMIT 1000)")
            await db.commit()

    async def get_cookie_sync_logs(self, limit=100):
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("SELECT * FROM cookie_sync_logs ORDER BY id DESC LIMIT ?", (max(1,min(limit,1000)),))
    ...
    async def get_logs(self, limit: int = 100, token_id: Optional[int] = None, include_payload: bool = False):
        ...
        async with self._connect() as db:
            ...
            # Select only classification metadata, keeping list payloads lightweight.
            payload_columns += "CASE WHEN json_valid(rl.request_body) THEN json_extract(rl.request_body, '$.model') END AS request_model,"
            payload_columns += "CASE WHEN json_valid(rl.request_body) THEN json_extract(rl.request_body, '$.has_images') END AS request_has_images,"
