...
from .flow_web import FlowWebClient, FlowWebError
...
class TokenManager:
    ...
    def __init__(self, db: Database, flow_client: FlowClient):
        self.db = db
        self._web_sync_lock = asyncio.Lock()
        self._flow_validation_locks = {}
    ...
    def _normalize_protocol_mode(self, value: Optional[str]) -> str:
        ...
        return mode if mode in {"protocol", "flow_web"} else "session"

    async def get_flow_web_client(self, google_cookies: str, proxy_url: Optional[str] = None):
        proxy = proxy_url or await ProxyManager(self.db).get_proxy_url()
        return FlowWebClient(google_cookies, proxy=proxy)

    def _flow_lock(self, token_id):
        return self._flow_validation_locks.setdefault(token_id, asyncio.Lock())

    async def record_flow_session_error(self, token, error):
        async with self._flow_lock(token.id):
            current = await self.db.get_token(token.id)
            if not current or current.google_cookies != token.google_cookies:
                return
            await self._save_flow_error(current, error)

    async def _save_flow_error(self, token, error):
        self._flow_validation_cache.pop(token.id, None)
        fields = dict(last_st_refresh_at=datetime.now(timezone.utc), last_st_refresh_result=str(error))
        if token.is_active and error.code in {"flow_session_expired", "flow_cookies_refresh_required", "flow_account_mismatch"}:
            fields.update(is_active=False, ban_reason="flow_session_invalid", banned_at=datetime.now(timezone.utc))
        await self.db.update_token(token.id, **fields)

    async def _validate_flow_token(self, token, force=False):
        async with self._flow_lock(token.id):
            token = await self.db.get_token(token.id) or token
            key = (token.google_cookies, token.proxy_url)
            cached = self._flow_validation_cache.get(token.id)
            if not force and cached and cached[0] == key and time.monotonic() - cached[1] < 60:
                return token
            try:
                client = await self.get_flow_web_client(token.google_cookies, token.proxy_url)
                identity = await client.authenticate()
                if identity["email"].lower() != token.email.lower():
                    raise FlowWebError("flow_account_mismatch", "Google 会话账号已切换，请同步对应账号的 Cookies")
                credits = await client.get_credits()
            except FlowWebError as error:
                await self._save_flow_error(token, error)
                raise
            await self.db.update_token(token.id, credits=credits["credits"], user_paygate_tier=credits["userPaygateTier"],
                                       last_st_refresh_at=datetime.now(timezone.utc), last_st_refresh_result="新版 Flow 会话已验证")
            token.credits = credits["credits"]
            token.user_paygate_tier = credits["userPaygateTier"]
            self._flow_validation_cache[token.id] = (key, time.monotonic())
            return token

    async def sync_flow_web_token(self, google_cookies: str, *, auto_enable: bool = True,
                                  proxy_url: Optional[str] = None, expected_id: Optional[int] = None):
        client = await self.get_flow_web_client(google_cookies, proxy_url)
        identity = await client.authenticate()
        email = identity["email"]
        credits = await client.get_credits()
        # Store the filtered cookie jar, never analytics or unrelated origins.
        serialized = json.dumps(client.cookies, separators=(",", ":"))
        async with self._web_sync_lock:
            existing = await self.db.get_token_by_email(email)
            if expected_id is not None and (not existing or existing.id != expected_id):
                raise FlowWebError("flow_account_mismatch", "Cookie 所属账号与正在编辑的记录不同")
            fields = dict(protocol_mode="flow_web", google_cookies=serialized,
                          at=None, at_expires=None, last_st_refresh_at=datetime.now(timezone.utc),
                          last_st_refresh_result="新版 Flow 会话已验证",
                          credits=credits["credits"], user_paygate_tier=credits["userPaygateTier"])
            if proxy_url is not None:
                fields["proxy_url"] = proxy_url
            if existing:
                async with self._flow_lock(existing.id):
                    existing = await self.db.get_token(existing.id) or existing
                    if auto_enable and existing.ban_reason == "flow_session_invalid":
                        fields.update(is_active=True, ban_reason=None, banned_at=None)
                    await self.db.update_token(existing.id, **fields)
                    self._flow_validation_cache.pop(existing.id, None)
                    return await self.db.get_token(existing.id), "updated"
            project_id = await client.create_project("Flow2API")
            token = Token(st="flow-web:" + str(uuid.uuid4()), email=email,
                          current_project_id=project_id, current_project_name="Flow2API",
                          **fields)
            token.id = await self.db.add_token(token)
            await self.db.add_project(Project(project_id=project_id, token_id=token.id, project_name="Flow2API"))
            return token, "added"
    ...
    async def enable_token(self, token_id: int):
        ...
        # Enable the token
        async with self._flow_lock(token_id):
            self._flow_validation_cache.pop(token_id, None)
    ...
    async def disable_token(self, token_id: int):
        ...
        async with self._flow_lock(token_id):
            self._flow_validation_cache.pop(token_id, None)
            await self.db.update_token(token_id, is_active=False, ban_reason="manual")
    ...
    async def ensure_valid_token(self, token: Optional[Token], *, force: bool = False) -> Optional[Token]:
        """确保 token 的 AT 可用，并在必要时返回刷新后的最新对象。"""
        if not token:
            return None

        if getattr(token, "protocol_mode", "session") == "flow_web":
            return await self._validate_flow_token(token, force=force)
    ...
    async def _refresh_at_inner(self, token_id: int) -> bool:
        ...
        async with refresh_lock:
            ...
            if getattr(token, "protocol_mode", "session") == "flow_web":
                await self.ensure_valid_token(token, force=True)
                return True
    ...
    async def ensure_project_exists(self, token_id: int) -> str:
        ...
        async with project_lock:
            ...
            if getattr(token, "protocol_mode", "session") == "flow_web":
                ...
                client = await self.get_flow_web_client(token.google_cookies, token.proxy_url)
                project_id = await client.create_project("Flow2API")
                await self.db.add_project(Project(project_id=project_id, token_id=token_id, project_name="Flow2API"))
                await self.db.update_token(token_id, current_project_id=project_id, current_project_name="Flow2API")
                return project_id
