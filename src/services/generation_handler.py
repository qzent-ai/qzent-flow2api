...
from .log_inputs import collect_log_inputs, cache_uploaded_video
...
MODEL_CONFIG = {
    ...
    # Omni Flash 时长档位:每个档位是独立上游 model_key,横竖屏共享 key
    "omni_4s": {
        "type": "video",
        "video_type": "omni",
        ...
        "use_v2_model_config": True,
        "allow_tier_upgrade": False,
        "reference_model_key": "abra_r2v_4s",
    ...
    "omni_6s": {
        ...
        "use_v2_model_config": True,
        "allow_tier_upgrade": False,
        "reference_model_key": "abra_r2v_6s",
    ...
    "omni_10s": {
        ...
        "use_v2_model_config": True,
        "allow_tier_upgrade": False,
        "reference_model_key": "abra_r2v_10s",
    ...
    # ========== 视频物体/场景编辑 (Edit - abra_edit) ==========
    # 2026-08-25 实机抓包:插入/移除均走 video:batchAsyncGenerateVideoEditVideo,
    # videoModelKey=abra_edit,请求不带 useV2ModelConfig;支持 0-1 张参考图(插入)。
    "flow_edit": {
        "type": "video",
        "video_type": "edit",
        "model_key": "abra_edit",
...
class GenerationHandler:
    ...
    async def _resolve_media_owner_token(self, media_id: str, fallback: Token) -> Token:
        """编辑/续写的源视频归属生成它的 Google 账号,必须路由回同一 token。"""
        owner_id = await self.db.get_token_id_by_media_generation_id(media_id)
        if owner_id is None:
            raise ValueError(f"源视频 {media_id[:8]}... 不属于任何已知 token，无法编辑")
        if owner_id == fallback.id:
            return fallback
        owner = await self.db.get_token(owner_id)
        if owner is None or not owner.is_active or not owner.video_enabled:
            raise ValueError(f"源视频所属 token {owner_id} 已禁用，无法编辑")
        debug_logger.log_info(f"[AFFINITY] 源视频归属 token {owner_id}，切换到属主 token")
        return owner

    async def upload_video_for_edit(self, data: bytes, content_type: str) -> str:
        """上传外部视频换取 Flow mediaId，并登记 token 归属（编辑亲和）。"""
        token = await self.load_balancer.select_token(
            for_image_generation=False, for_video_generation=True
        )
        if token is None:
            raise Exception(
                "没有可用的Token进行视频上传。所有Token都处于禁用、冷却、锁定或已过期状态。"
            )
        project_id = await self.token_manager.ensure_project_exists(token.id)
        if getattr(token, "protocol_mode", "session") == "flow_web":
            client = await self.token_manager.get_flow_web_client(token.google_cookies, token.proxy_url)
            media_id = await client.upload_video(project_id, data, content_type)
        else:
        ...
        # 归属登记：tasks 表记 token_id + media_generation_id，
        # 后续 edit:// 经 get_token_id_by_media_generation_id 路由回同一 token
        ...
        source_url = await cache_uploaded_video(self, data, content_type)
        await self.db.update_task(task_id, media_generation_id=media_id, result_urls=[source_url] if source_url else [])
        return media_id
    ...
    async def handle_generation(
        ...
        edit_directive: Optional[Dict[str, Any]] = None,
    ) -> AsyncGenerator:
        """统一生成入口
        ...
        request_operation = generation_operation(model_config, bool(images))
        prompt_for_log = prompt if len(prompt) <= 2000 else f"{prompt[:2000]}...(truncated)"
        request_payload = {
            "model": model,
            "prompt": prompt_for_log,
            "has_images": images is not None and len(images) > 0,
            "inputs": await collect_log_inputs(self, images, edit_directive, video_media_id),
        }
        ...
        if not token:
            ...
            yield self._create_error_response(error_msg, status_code=503,
                                              code="flow_web_generation_unsupported" if "flow_web_generation_unsupported" in error_msg else "generation_failed")
            return

        debug_logger.log_info(f"[GENERATION] 已选择Token: {token.id} ({token.email})")
        pending_token_state["active"] = True
        pending_token_state["id"] = token.id
        ...
        try:
            # 编辑/续写:源视频归属特定账号,切到属主 token
            affinity_media_id = (
                edit_directive["media_id"] if edit_directive else video_media_id
            )
            if affinity_media_id:
                try:
                    token = await self._resolve_media_owner_token(affinity_media_id, token)
                except ValueError as e:
                    error_msg = str(e)
                    if stream:
                        yield self._create_stream_chunk(f"{error_msg}\n")
                    self._mark_generation_failed(generation_result, error_msg)
                    # 机器可读 code:产品层据此识别亲和失败并触发重传自救
                    yield self._create_error_response(
                        error_msg, status_code=400, code="media_owner_unavailable"
                    )
                    return

            if getattr(token, "protocol_mode", "session") == "flow_web":
                from .flow_web_generation import generate_video, generate_image
                ...
                if not supports_model_for_tier(model, token.user_paygate_tier):
                    ...
                    raise FlowWebError("model_tier_unavailable", "当前账号等级不支持此模型", 403)
                project_id = await self.token_manager.ensure_project_exists(token.id)
                generate = generate_image if generation_type == "image" else generate_video
                async for chunk in generate(
                    self, token, project_id, model_config, prompt, images, stream,
                    generation_result, response_state, request_log_state, edit_directive,
                ):
                    yield chunk
            else:
                # 3. 确保AT有效
            ...
            if token:
                from .flow_web import FlowWebError
                if isinstance(e, FlowWebError) and getattr(token, "protocol_mode", "session") == "flow_web" and e.code in {"flow_session_expired", "flow_cookies_refresh_required", "flow_account_mismatch", "flow_session_unverified", "flow_connection_failed", "flow_http_error", "flow_access_denied"}:
                    await self.token_manager.record_flow_session_error(token, e)
            ...
            if stream:
                yield self._create_stream_chunk(f"错误: {error_msg}\n")
            yield self._create_error_response(error_msg, status_code=getattr(e, "status_code", 500), code=getattr(e, "code", "generation_failed"))
        finally:
            if pending_token_state.get("active") and token and self.load_balancer:
                await self.load_balancer.release_pending(
                    pending_token_state.get("id", token.id),
                    for_image_generation=(generation_type == "image"),
                    for_video_generation=(generation_type == "video"),
                )
                pending_token_state["active"] = False
    ...
    def _should_count_token_error(self, error: Exception) -> bool:
        ...
        if isinstance(error, ImportError):
            return False
        if isinstance(error, FlowWebError) and (
            error.code.startswith("invalid_") or error.code in {
                "flow_operation_unsupported", "flow_cache_failed", "recaptcha_failed",
                "flow_captcha_provider_unsupported", "flow_invalid_request",
                "flow_response_invalid", "flow_submission_unconfirmed",
                "flow_media_url_invalid", "flow_upload_url_invalid",
                "media_owner_unavailable", "model_tier_unavailable", "flow_connection_failed",
                "flow_session_expired", "flow_cookies_refresh_required", "flow_session_unverified",
                "flow_account_mismatch", "flow_access_denied", "flow_http_error",
            }
        ):
            return False
    ...
    async def _handle_video_generation(
        ...
        try:
            ...
            # Edit: 视频编辑 - 最多 1 张参考图 (min_images=0,无需下限校验)
            elif video_type == "edit":
                if max_images is not None and image_count > max_images:
                    error_msg = f"视频编辑最多支持 {max_images} 张参考图，当前提供了 {image_count} 张"
                    if stream:
                        yield self._create_stream_chunk(f"{error_msg}\n")
                    self._mark_generation_failed(generation_result, error_msg)
                    yield self._create_error_response(error_msg, status_code=400)
            ...
            # Edit: 视频物体/场景编辑 (abra_edit)
            elif video_type == "edit":
                if not edit_directive:
                    error_msg = "视频编辑需要提供源视频，请在 image_url 中传入 edit://VIDEO_MEDIA_ID?start_frame=0&end_frame=240"
                    if stream:
                        yield self._create_stream_chunk(f"{error_msg}\n")
                    self._mark_generation_failed(generation_result, error_msg)
                    yield self._create_error_response(error_msg, status_code=400)
                ...
                # 插入物体:0-1 张参考图,沿用 R2V 的 handler 端上传模式,额外保留 handle
                edit_references = []
                for image_bytes in images or []:
                    media_id, handle = await self.flow_client._upload_image_impl(
                        token.at, image_bytes,
                        aspect_ratio=model_config["aspect_ratio"],
                        project_id=project_id,
                    )
                    edit_references.append({"media_id": media_id, "handle": handle})

                debug_logger.log_info(f"[EDIT] 编辑视频: {edit_directive['media_id']}")
                if stream:
                    yield self._create_stream_chunk("视频编辑任务提交中...\n")
                result = await self.flow_client.generate_video_edit(
                    ...
                    model_key=model_config["model_key"],
                    aspect_ratio=model_config["aspect_ratio"],
                    video_media_id=edit_directive["media_id"],
                    start_frame=edit_directive["start_frame"],
                    end_frame=edit_directive["end_frame"],
                    reference_images=edit_references or None,
                    user_paygate_tier=normalized_tier,
                    token_id=token.id,
                    token_video_concurrency=token.video_concurrency,
                )
    ...
    def _create_completion_response(self, content: str, media_type: str = "image", is_availability_check: bool = False, media_id: Optional[str] = None) -> str:
        """创建非流式响应
        ...
        if is_availability_check:
            ...
            if media_type == "video":
                ...
                media_attr = f" data-media-id='{escape(media_id, quote=True)}'" if media_id else ""
                formatted_content = f"```html\n<video src='{content}'{media_attr} controls></video>\n```"
            else:  # image
