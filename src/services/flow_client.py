...
class FlowClient:
    ...
    async def upload_image(
        ...
        """上传图片,返回mediaId(签名不变,兼容现有调用方)"""
        media_id, _ = await self._upload_image_impl(at, image_bytes, aspect_ratio, project_id)
        return media_id
    ...
    async def _upload_image_impl(
        ...
        """上传图片,返回 (mediaId, upload_file_name)

        upload_file_name 为本次上传构造的 flow2api_upload_<ms>.<ext> 文件名,
        供结构化提示词的 reference handle 使用(与 Flow 网页端行为一致)。
        ...
        for retry_attempt in range(max_retries):
            try:
                ...
                if media_id:
                    return media_id, upload_file_name
            ...
            try:
                ...
                if media_id:
                    # legacy 回退路径不返回相同语义的 fileName，统一返回新接口构造的文件名
                    return media_id, upload_file_name
    ...
    # ========== 视频上传 (resumable X-Goog-Upload 协议) ==========

    # 上传专用 host(注意 .sandbox. 子域),与生成接口的 aisandbox-pa.googleapis.com 不同。
    # Upload flow follows the account-scoped upstream media protocol.
    FLOW_UPLOAD_BASE_URL = "https://aisandbox-pa.sandbox.googleapis.com"
    ...
    async def upload_video(
        ...
        *,
        ...
        """上传视频到 Flow,返回 mediaId(可用于 edit:// 编辑)。

        标准 Google resumable upload:start 拿会话 URL,单块 upload+finalize 收口。
        实测仅需 Bearer AT,无需 reCAPTCHA。
        """
        start_url = (
            f"{self.FLOW_UPLOAD_BASE_URL}/upload/v1/flow/upload/video/"
            f"{project_id}?upload_protocol=resumable"
        )
        async with httpx.AsyncClient(
            ...
            if start_resp.status_code != 200:
                raise Exception(
                    f"Flow video upload start failed: HTTP {start_resp.status_code}: "
                    f"{start_resp.text[:200]}"
                )
            upload_url = start_resp.headers.get("X-Goog-Upload-URL")
            if not upload_url:
                raise Exception(
                    "Flow video upload start failed: missing X-Goog-Upload-URL"
                )
            ...
            if final_resp.status_code != 200:
                raise Exception(
                    f"Flow video upload failed: HTTP {final_resp.status_code}: "
            ...
            try:
                ...
                raise Exception(
                    f"Invalid upload response: {final_resp.text[:200]}"
                ) from exc

        media_id = result.get("mediaId") or (result.get("media") or {}).get("name")
        if not media_id:
            raise Exception(
                f"Invalid upload response: missing media id, keys={list(result.keys())}"
            )
        return media_id
    ...
    async def generate_video_edit(
        ...
        """视频编辑,基于已生成视频的指定帧区间做对象/场景编辑

        Args:
            at: Access Token
            project_id: 项目ID
            prompt: 编辑提示词
            model_key: abra_edit 等
            aspect_ratio: 视频宽高比
            video_media_id: 源视频的 mediaGenerationId
            start_frame: 编辑起始帧(0-300)
            end_frame: 编辑结束帧(0-300)
            reference_images: 参考图列表,元素为 {"media_id": str, "handle": str}
            user_paygate_tier: 用户等级
        ...
        while retry_attempt < max_retries:
            ...
            if reference_images:
                for ref in reference_images:
                    prompt_parts.append({
                        "reference": {
                            "media": {"handle": ref["handle"], "mediaId": ref["media_id"]}
                        }
                    })
            ...
            edit_request: Dict[str, Any] = {
                ...
                "aspectRatio": aspect_ratio,
                "textInput": {"structuredPrompt": {"parts": prompt_parts}},
                "videoModelKey": model_key,
                "seed": request_seed,
            ...
            if reference_images:
                edit_request["referenceImages"] = [
                    {"mediaId": ref["media_id"], "imageUsageType": "IMAGE_USAGE_TYPE_ASSET"}
                    for ref in reference_images
                ]
            ...
            json_data = {
                ...
                # 注意:编辑端点不带 useV2ModelConfig(2026-08-25 抓包确认)
            }

            # Debug: 打印请求体用于调试
            import json as _json
            debug_logger.log_info(f"[VIDEO EDIT] Request URL: {url}")
            debug_logger.log_info(f"[VIDEO EDIT] Request JSON: {_json.dumps(json_data, indent=2, ensure_ascii=False)[:2000]}")
    ...
    async def _get_api_captcha_token(
        ...
        if website_url is not None and (urlparse(website_url).scheme != "https" or urlparse(website_url).netloc != "flow.google.com"):
            raise ValueError("Invalid captcha website origin")
        website_url = website_url or self._build_flow_project_page_url(project_id)
        ...
        try:
            ...
            if not resolved_proxy_url and self.proxy_manager:
                try:
                    ...
                    if resolved_proxy_url:
                        if resolved_proxy_url.startswith(("socks5://", "socks5h://")):
                    ...
                    debug_logger.log_warning(f"[reCAPTCHA {method}] Failed to get proxy: {type(e).__name__}")
            ...
            async with AsyncSession() as session:
                ...
                for create_attempt in range(max_create_attempts):
                    ...
                    if method == "yescaptcha" and error_code in {"ERROR_NO_SLOT_AVAILABLE", "ERROR_NO_SLOT_AVAILABLE_BLOCK"}:
                        ...
                        if create_attempt < max_create_attempts - 1:
                            ...
                            continue
                    debug_logger.log_error(f"[reCAPTCHA {method}] Failed to create task")
                    return None
                ...
                for i in range(40):
                    ...
                    debug_logger.log_info(f"[reCAPTCHA {method}] polling #{i+1}: status={result_json.get('status')}")
            ...
            debug_logger.log_error(f"[reCAPTCHA {method}] error: {type(e).__name__}")
            return None
